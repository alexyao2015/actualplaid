import { getAppConfigFromEnv } from "./config";
import fs from "fs";
import inquirer from "inquirer";
import { actual, q } from "@actual-app/api";
    ...
    if (!id) {
      id = (
        await inquirer.prompt({
          name: "budget_id",
          message: `This is your (${config.get(
            "user"
          )}) first time using this user, what is your budget sync Id? (Can be found in advanced settings on Actual as the 'Sync Id')`,
        })
      ).budget_id;
 ...
 */
export function listAccounts(actualInstance) {
 ...
 * @param {*} accountId
 */
export async function getLastTransactionDate(actualInstance, accountId) {
  const monthAgo = new Date();
  monthAgo.setMonth(monthAgo.getMonth() - 1);

  const transactions = await actualInstance.getTransactions(
    accountId,
    monthAgo,
    new Date()
  );
...
const ABN_AMRO_TRANSACTION_MAPPER = (accountId) => (transaction) => {
  ...
  if (description.includes("TRTP")) {
    ...
    if (
      splitted[2].includes("iDEAL") ||
      splitted[2].includes("SEPA OVERBOEKING")
    ) {
...
export const transactionMapper = (accountId, bank) => {
  if (map[bank]) {
...
export async function getBalance(actualInstance, accountId) {
  const balance = await actualInstance.runQuery(
    q("transactions")
 ...
 * @param {typeof actual} actualInstance
 */
export async function finalize(actualInstance) {
