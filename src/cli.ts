...
import { getAppConfigFromEnv, getConf } from "./config.js";
import {
  initialize,
...
const printSyncedAccounts = () => {
  ...
  const linkedToActual = Object.entries(actualData).map(
    ([actualId, { plaidAccount }]) => {
      return { plaid: plaidAccount.account_id, actual: actualId };
    }
  );
  ...
  console.log(
    "The following Plaid accounts are linked to this app, but not to Actual:"
  );
  console.table(
...
export const actualPlaid = async (
  command: string,
  flags: { account: string; since: string }
) => {
  if (!command) {
  ...
  if (command === "config") {
    ...
    if (Object.keys(syncingData).length) {
      ...
      const accountsToSync = Object.entries(syncingData).filter(
        ([_, account]) => !flags.account || account.actualName === flags.account
      );
      ...
      for (let [actualId, account] of accountsToSync) {
        ...
        if (startDate === endDate) {
          console.log(
            ...
            "because it was already imported today"
          );
        } else {
          console.log(
          ...
          const transactionsResponse = await cachedTransaction(
            account.plaidToken,
            startDate
          );
          const transactionsForThisAccount =
            transactionsResponse.transactions.filter(
              (transaction) =>
    ...
    if (Object.keys(plaidAccounts).length == 0) {
      console.log(
        "There are no accounts linked to Plaid that are not already in Actual. Please link at least one new account to continue."
      );
      plaidAccounts = await startLinkingPlaid();
    } else {
      console.log(
        "The following accounts are linked to Plaid, but not to Actual:"
      );
      console.table(
    ...
    const accountsInTheActualBudget = await listAccounts(
    ...
    const { accountsToSync } = await inquirer.prompt({
      ...
      choices: accountsInTheActualBudget
        ...
        .filter(
          ({ value }) => !linkedToActual.find(({ actual }) => actual === value)
        ),
    });
    ...
    for (let [actualId, account] of Object.entries(syncingData)) {
      ...
      const plaidBalanceInformation = await plaidClient.getBalance(
        account.plaidToken,
        {
      ...
      const balanceFromPlaid =
        plaidBalanceInformation.accounts[0].balances.current;
      const actualConverted = actual.utils.integerToAmount(balanceFromActual);

      console.log(
        `Checking balance for account: ${account.actualName} (${account.plaidBankName})`
      );
      ...
      if (actualConverted !== balanceFromPlaid) {
        throw new Error(
          `Balance for account ${account.actualName} (${account.plaidBankName}) does not match. Actual: ${balanceFromActual} Plaid: ${balanceFromPlaid}`
        );
      }
    }
  } else if (command === "ls") {
...
fastify.post("/get_access_token", async (request, reply) => {
  ...
  try {
    ...
    const {
      accounts,
      item: { institution_id },
    } = await plaidClient.getAccounts(access_token);
    const {
      institution: { name },
    } = await plaidClient.getInstitutionById(institution_id);
